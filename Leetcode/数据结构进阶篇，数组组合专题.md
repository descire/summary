...
&emsp;&emsp;寻找可行解的题型，多可以采用*搜素回溯*的方法来解决。定义递归函数进行枚举：
...
&emsp;&emsp;这道题目相比较上一题加了一些限制条件：*每个数字只使用一次和不重复组合*。

&emsp;&emsp;*只要在枚举的过程中都移动 index 下标即可保证每个数字只使用一次*。

&emsp;&emsp;针对不重复组合，第一时间会想到通过*哈希表来对结果去重*，但是这种方式并*不能降低算法本身的时间复杂度*。

&emsp;&emsp;出现重复组合的根本原因在于：*每层枚举的过程中，重复处理了已经处理过的数字*。

&emsp;&emsp;为了能够方便过滤到本层重复处理的数字，需要对数组进行*排序预处理*。
...
&emsp;&emsp;本道题求解最终组合的排列个数，如果还是采用回溯枚举的方式时间复杂度会非常的高，所以本题只要求求解可行解的个数，而非具体的排列。
...
&emsp;&emsp;在动态规划的过程中，*通过 dp[sum] 消除了回溯中 backtrack(sum) 这样的重叠子问题的消耗*。
